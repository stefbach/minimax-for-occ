...
export default async function WorkflowsPage() {
  ...
  return (
    <>
      ...
      {error && (
        <div className="card" style={{ borderColor: "var(--bad)", color: "var(--bad)" }}>
          ...
          <div className="muted" style={{ marginTop: 6 }}>
            ...
            <span className="kbd">N8N_API_KEY</span> sont définis.
          </div>
        </div>
      )}
      ...
      {!error && (
        <>
          <Section title="Actifs" rows={active} />
          <Section title="Inactifs" rows={inactive} dim />
        </>
      )}
    </>
  );
    ...
    <section style={{ marginBottom: 18 }}>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden", opacity: dim ? 0.6 : 1 }}>
        <table className="list">
          <thead>
            <tr><th>Nom</th><th>Webhooks</th><th>Tags</th></tr>
          </thead>
          <tbody>
            {rows.map((w) => (
              <tr key={w.id}>
