...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const sb = supabaseServer();
  // Tenant-safety: verify the parent agent belongs to this org before
  // letting the caller insert documents that would inherit its agent_id.
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const sb = supabaseServer();
  let q = sb.from("documents").delete().eq("agent_id", id).eq("org_id", orgId);
