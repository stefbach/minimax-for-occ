...
${dynamic
  ? `- Cette campagne est CONTINUE (tirée de la table « ${ctx.table_label ?? "sélectionnée"} »). Tu PEUX définir include_statuses (colonne statut : ${ctx.status_column ?? "inconnue"}), max_new_per_day, wave_size et relance_days_after_first.${typeof ctx.detected_relance_phases === "number" ? ` La table expose ${ctx.detected_relance_phases} phase(s) de relance détectée(s) ; n'en propose pas davantage.` : ""}
- CADENCE relance_days_after_first : tableau de délais CUMULATIFS en jours ouvrés depuis le début. Format : [0, 2, 4] = J1 démarre immédiatement (0), J3 = 2 jours ouvrés après J1, J5 = 2 jours ouvrés après J3. TOUJOURS commencer par 0 (J1 est le 1er appel, il ne attend pas). Pour une cadence J1/J3/J5 envoyer EXACTEMENT 3 valeurs : [0, 2, 4]. Pour J1/J3 seulement : [0, 2]. Ne jamais envoyer [1,3,5] ni [2,4] — le premier élément doit être 0.
